...
# Primero, recrear la tabla basándose en el DataFrame
with engine.connect() as conn:
    # Eliminar tabla existente
    conn.execute(text("DROP TABLE IF EXISTS pac_operations CASCADE"))
    conn.commit()

# Crear la tabla nueva basada en el DataFrame
...
with engine.connect() as conn:
    ...
    try:
        conn.execute(text("ALTER TABLE pac_operations ADD PRIMARY KEY (proposta)"))
        conn.commit()
        print("✅ Clave primaria agregada")
    except:
        print("⚠️  No se pudo agregar clave primaria (posibles duplicados)")

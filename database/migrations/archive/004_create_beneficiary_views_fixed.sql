-- Migration: Create beneficiary and enhanced analytics views (FIXED)
-- Date: 2025-01-06
-- Description: Adds views for beneficiary tracking and analytics - works with current table structure
...
SELECT 
    ...
    ROUND(AVG(EXTRACT(days FROM CURRENT_DATE - ps.data_inicio_obra))::numeric, 0) as media_dias_em_obra
FROM projeto_status ps
WHERE ps.tipo_programa = 'MCMV-HIS'
GROUP BY ps.programa, ps.uf
ORDER BY ps.programa, ps.uf;

-- 4. Timeline Analysis View (fixed column names)
...
WITH prazos AS (
    SELECT 
        ...
        EXTRACT(days FROM CURRENT_DATE - data_inicio_obra::date) as dias_desde_inicio,

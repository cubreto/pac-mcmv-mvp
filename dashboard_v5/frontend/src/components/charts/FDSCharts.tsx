/**
 * MCMV Dashboard v5 - FDS Chart Components
 * Specialized charts for FDS program analysis
 */
...
// FDS program colors (purple theme)
const FDS_COLORS = {
  primary: '#9733FF',
  secondary: '#7C3AED',
  accent: '#A855F7',
...
const STATUS_COLORS = [
  '#9733FF', '#7C3AED', '#A855F7', '#8B5CF6', '#C084FC', '#E9D5FF'
]

interface FDSChartProps {
  filters?: {
    regiao?: string
    state?: string
    municipality?: string
...
export function FDSRegionChart({ filters }: FDSChartProps) {
...
export function FDSStatusChart({ filters }: FDSChartProps) {
...
export function FDSTimelineChart({ filters }: FDSChartProps) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Line 
            type="monotone" 
            dataKey="total_uh" 
            stroke={FDS_COLORS.primary}
            strokeWidth={3}
            dot={{ fill: FDS_COLORS.primary, strokeWidth: 2, r: 4 }}
            name="UH Previstas"
          />
        </LineChart>
      </ResponsiveContainer>
...
export function FDSFinancialTable({ filters }: FDSChartProps) {

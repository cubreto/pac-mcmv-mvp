/**
 * MCMV Dashboard v5 - FAR Chart Components
 * Specialized charts for FAR program analysis
 */
...
import { 
  useFARRegionStatusChart, 
  useFARStatusDonutChart, 
  useFARTimelineChart,
  useFARFinancialTable 
} from '../../api/hooks'
...
const STATUS_COLORS = [
  '#2563EB', '#1D4ED8', '#60A5FA', '#3B82F6', '#93C5FD', '#DBEAFE'
]

interface FARChartProps {
  filters?: {
    regiao?: string
    state?: string
    municipality?: string
...
export function FARRegionChart({ filters }: FARChartProps) {
...
export function FARStatusChart({ filters }: FARChartProps) {
...
export function FARTimelineChart({ filters }: FARChartProps) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Line 
            type="monotone" 
            dataKey="total_uh" 
            stroke={FAR_COLORS.primary}
            strokeWidth={3}
            dot={{ fill: FAR_COLORS.primary, strokeWidth: 2, r: 4 }}
            name="UH Previstas"
          />
        </LineChart>
      </ResponsiveContainer>
...
export function FARFinancialTable({ filters }: FARChartProps) {
  const { data, isLoading, error } = useFARFinancialTable(filters)
